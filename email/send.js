...
async function handleSendEmail(args) {
  const { to, cc, bcc, subject, body, importance = 'normal', saveToSentItems = true, sharedMailbox } = args;
  ...
  try {
    ...
    // Make API call to send email - use shared mailbox if specified
    const sendEndpoint = sharedMailbox ? `users/${sharedMailbox}/sendMail` : 'me/sendMail';
    await callGraphAPI(accessToken, 'POST', sendEndpoint, emailObject);
